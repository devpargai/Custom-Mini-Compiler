...
void SemanticAnalyzer::analyze(ParseNode* root) {
    ...
    errors.clear();
    loopDepth = 0;
    currentReturnType = "intt"; // Assume mainn returns intt for now
    traverse(root);
}
...
void SemanticAnalyzer::traverse(ParseNode* node) {
    ...
    switch (node->type) {
        case DECLARATION_NODE: {
            ...
            // Check initializer expression type compatibility
...
string SemanticAnalyzer::getExprType(ParseNode* node) {
    ...
    // ✅ If the value is a string literal like "hello"
    ...
    // ✅ Handle operators: infer type based on children
    if (node->value == "+" || node->value == "-" || node->value == "*" || node->value == "/" ||
        ...
        if (node->value == "&&" || node->value == "||") {
            return "intt";  // Logical operators return intt
        }
        ...
        return leftType; // Arithmetic
    }

    // ✅ If it might be a variable
    if (symbolTable.count(node->value)) {
        return symbolTable[node->value].type;
    }
