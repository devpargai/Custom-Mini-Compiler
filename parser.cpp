...
void Parser::error(const string& message) {
    ...
    cerr << err << endl << flush;
    cout << err << endl; // Debug visibility
    exit(1);
}
...
ParseNode* Parser::parseProgram() {
    ...
    // Handle empty block or statements until '}'
    if (!isAtEnd() && peek().value != "}") {
        node->children.push_back(parseStmtList());
    }

    // Ensure there's a closing '}'
    if (!match(DELIMITER, "}")) {
...
ParseNode* Parser::parseStmt() {
    ...
    if (match(IDENTIFIER)) {
        ...
        if (match(OPERATOR, "=")) {
            ...
            // Build assignment node with LHS var name in .value
...
ParseNode* Parser::parseExpr() {
    return parseLogic();  // Entry point now supports logic
}
...
ParseNode* Parser::parsePrimary() {
    ...
    if (match(NUMBER)) {
        // Return NUMBER_NODE instead of generic EXPRESSION_NODE
        return new ParseNode{NUMBER_NODE, previous().value, {}};
    }

    if (match(STRING_LITERAL)) {
        // Return STRING_LITERAL_NODE (if defined), else EXPRESSION_NODE with string value
